...
智能合约在编译后会使用函数选择器（function selector）来标识函数。函数选择器由函数名称与参数列表进行哈希计算后所得到的前 4 个字节（bytes）。如果两个不同函数的哈希前 32 位恰好一致，它们将生成相同的 4 字节选择器，即使函数名称和参数完全不同。

Solidity 编译器会检测同一合约内部的函数选择器重复问题，但如果冲突发生在不同合约之间（例如代理合约与实现合约之间），编译器无法阻止这种冲突。

大多数代理模式都有可能遭遇函数选择器冲突，但并非全部如此。尤其是在 UUPS 代理中，由于所有自定义逻辑都集中在实现合约中，代理本身极为精简，因此出现函数选择器冲突的可能性更低。

**测试步骤**

要检测此类漏洞，可以将代理合约与实现合约的所有函数选择器收集并比较，检查是否存在冲突。

常用工具包括：

- solc
  `solc --hashes MyContract.sol`
  列出所有函数选择器。

- [Slither - Function ID printer](https://github.com/crytic/slither/wiki/Printer-documentation#function-id)
  输出所有函数选择器（Function IDs）。
...
既然合约的构造函数（constructor）在部署时会自动执行，为什么代理合约仍然需要一个 `initialize` 函数？OpenZeppelin 在[此文档](https://docs.openzeppelin.com/upgrades-plugins/1.x/proxies#the-constructor-caveat)中对此进行了详细说明。

构造函数只会在合约部署时运行一次。然而，实现合约（也称逻辑合约）的构造函数永远无法在代理合约的执行上下文中被调用。
由于实现合约的构造函数始终在其自身环境中执行，它无法为代理合约的存储进行初始化。

因此，实现合约必须在代理的存储上下文中设置 `_initialized` 等初始状态，这就是为什么实现合约必须提供 `initialize` 函数，并且初始化必须通过代理来调用，而不是通过实现合约本身。
由此也引入一个潜在的竞争条件（race condition）：初始化必须在部署代理之后、且在代理被外部使用之前完成，否则可能被攻击者抢先执行。

为避免此类风险，可以在 `initialize` 函数中加入基于地址的权限控制修饰符（modifier），使初始化只能由特定的 `msg.sender` 执行。

在 [OpenZeppelin 4.1.0 至 4.3.2](https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/security/advisories/GHSA-q4h9-46xg-m3x9) 版本之间，曾出现未初始化 UUPS Proxy 的特定漏洞。

**测试步骤**

要检测此类漏洞，可以检查用于初始化状态判定的变量（例如 [`_initialized`](https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/25aabd286e002a1526c345c8db259d57bdf0ad28/contracts/proxy/utils/Initializable.sol#L62)）在代理合约存储中的对应槽位，或其他表示“是否已初始化”的变量。

在使用 OpenZeppelin 的 `Initializable.sol` 时，文件中的私有变量 `_initialized` 用于记录初始化状态：

- `_initialized == 0` → 合约尚未初始化
- `_initialized == 1` → 合约已完成初始化

Slither 的 [slither-check-upgradeability](https://github.com/crytic/slither/wiki/Upgradeability-Checks) 工具包含多个与初始化相关的检测器，可用于辅助分析。
